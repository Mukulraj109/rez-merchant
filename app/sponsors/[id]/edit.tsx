...
import { socialImpactAdminService, Sponsor, UpdateSponsorData } from '@/services/api/socialImpact';
...
export default function EditSponsorScreen() {
  ...
  const loadSponsor = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const sponsor = await socialImpactAdminService.getSponsorById(id);
      populateForm(sponsor);
    } catch (error: any) {
      setErrorModal({
        visible: true,
        title: 'Error',
        message: error.message || 'Failed to load sponsor',
      });
    } finally {
      setLoading(false);
    }
  ...
  const populateForm = (sponsor: Sponsor) => {
    setName(sponsor.name);
    setBrandCoinName(sponsor.brandCoinName);
    setDescription(sponsor.description || '');
    setWebsite(sponsor.website || '');
    setContactName(sponsor.contactPerson?.name || '');
    setContactEmail(sponsor.contactPerson?.email || '');
    setContactPhone(sponsor.contactPerson?.phone || '');

    if (sponsor.logo) {
      setLogoUri(sponsor.logo);
      setLogoUrl(sponsor.logo);
    }
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const sponsorData: UpdateSponsorData = {
        name: name.trim(),
      ...
      await socialImpactAdminService.updateSponsor(id!, sponsorData);
      ...
      <>
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading sponsor...</Text>
          </View>
        </SafeAreaView>
      </>
    );
      ...
      <View style={styles.container}>
        ...
        <SafeAreaView style={styles.safeArea} edges={['top']}>
          ...
          <Animated.View entering={FadeIn.duration(600)} style={styles.header}>
            ...
            <Text style={styles.headerTitle}>Edit Sponsor</Text>
            <TouchableOpacity
